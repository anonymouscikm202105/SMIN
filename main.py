...
class Hope():
    ...
    def prepareModel(self):
        ...
        #gmi
        if self.args.gmi == 1:
            if self.args.gmi_graph_act == 'sigmoid':
                gmiGraphAct = nn.Sigmoid()
            elif self.args.gmi_graph_act == 'tanh':
                gmiGraphAct = nn.Tanh()
            print('GMI graph-level Act funciton: '+self.args.gmi_graph_act )
            self.ui_gmi = GMI(self.ui_graph,self.out_dim, self.out_dim, nn.PReLU(), gmiGraphAct,self.ui_graphAdj).cuda()
            self.opt = optim.Adam([
                {'params':self.model.parameters(),'weight_decay':0},
                {'params':self.ui_gmi.parameters(),'weight_decay':0},
                ],lr=self.args.lr)
        else:
        ...
        for user, item_i, item_j in self.train_loader:  
            ...
            if self.args.gmi == 1:
                ...
                if self.args.lambda1 != 0 or self.args.lambda2 != 0:
                    res = self.ui_gmi(self.allEmbed, self.ui_subGraphAdj, self.ui_subGraphAdj_Tensor,self.ui_subGraphAdj_Norm)
                    Mask = t.zeros((self.userNum+self.itemNum)).cuda()
                    Mask[user]=1
                    Mask[self.userNum+item_i] = 1
                    Mask[self.userNum+item_j] = 1
                    gmi_loss = self.args.lambda1*(((Mask*res[0]).sum()+(Mask*res[1]).sum())/t.sum(Mask))\
                        +self.args.lambda2*(((Mask*res[2]).sum()+(Mask*res[3]).sum())/t.sum(Mask)+res[4])
                    epoch_gmi_loss += gmi_loss.item()
                    loss = loss + gmi_loss 
            self.opt.zero_grad()
            loss.backward()
            self.opt.step()
...
if __name__ == '__main__':
    ...
    #gmi
    parser.add_argument('--gmi', type=int, default=1, help="whether use gmi model block")
    parser.add_argument('--gmi_graph_act',default='sigmoid',help='gmi graph activation function')
    parser.add_argument('--lambda1', type=float, default=0.06, help='weight of loss with gmi')
    parser.add_argument('--lambda2', type=float, default=0.002, help='weight of loss with gmi')
